...
local Theme = {
    ...
    SurfaceHighlight= Color3.fromRGB(28, 28, 35),
    Accent1         = Color3.fromRGB(255, 215, 0),   -- Gold main
    Accent2         = Color3.fromRGB(218, 165, 32),  -- Goldenrod
    GoldLight       = Color3.fromRGB(255, 223, 100),
...
-- ============================================================
-- FLOATING BAR (VEXIS HUB)
-- ============================================================
do
    local floatingBarGui = Instance.new("ScreenGui")
    floatingBarGui.Name = "VexisFloatingBar"
    floatingBarGui.ResetOnSpawn = false
    floatingBarGui.IgnoreGuiInset = true
    floatingBarGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    floatingBarGui.Parent = game:GetService("CoreGui")

    local barFrame = Instance.new("Frame")
    barFrame.Name = "BarFrame"
    barFrame.Size = UDim2.new(0, 280, 0, 40)
    barFrame.Position = UDim2.new(0.5, -140, 0.02, 10)
    barFrame.BackgroundColor3 = Theme.Background
    barFrame.BackgroundTransparency = 0.08
    barFrame.BorderSizePixel = 0
    barFrame.Parent = floatingBarGui
    ...
    barGradient.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(28, 25, 32)),
    ...
    glowStroke.Color = Theme.GoldLight
    glowStroke.Thickness = 3
    glowStroke.Transparency = 0.85
    glowStroke.Parent = barFrame
    ...
    titleText.Text = "VEXIS HUB"
    titleText.TextSize = 18
    titleText.TextColor3 = Theme.Accent1
    titleText.TextXAlignment = Enum.TextXAlignment.Center
    titleText.TextYAlignment = Enum.TextYAlignment.Center
    titleText.Parent = barFrame
    ...
    subText.Parent = barFrame

    local goldDot = Instance.new("Frame")
    goldDot.Name = "GoldDot"
    goldDot.Size = UDim2.new(0, 6, 0, 6)
    goldDot.Position = UDim2.new(0, 12, 0.5, -3)
    goldDot.BackgroundColor3 = Theme.Accent1
    goldDot.BorderSizePixel = 0
    goldDot.Parent = barFrame
    Instance.new("UICorner", goldDot).CornerRadius = UDim.new(1, 0)

    local goldDot2 = Instance.new("Frame")
    goldDot2.Name = "GoldDot2"
    goldDot2.Size = UDim2.new(0, 6, 0, 6)
    goldDot2.Position = UDim2.new(1, -18, 0.5, -3)
    goldDot2.BackgroundColor3 = Theme.Accent1
    goldDot2.BorderSizePixel = 0
    goldDot2.Parent = barFrame
    Instance.new("UICorner", goldDot2).CornerRadius = UDim.new(1, 0)

    task.spawn(function()
        while barFrame.Parent do
            for i = 0, 360, 2 do
                if not barFrame.Parent then break end
                goldDot.BackgroundColor3 = Color3.fromHSV((i / 360) * 0.15 + 0.12, 1, 1)
                goldDot2.BackgroundColor3 = Color3.fromHSV((i / 360) * 0.15 + 0.12, 1, 1)
                task.wait(0.02)
            end
        end
    end)
    ...
    local function pulseGlow()
        while barFrame.Parent do
            TweenService:Create(glowStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.75}):Play()
            task.wait(1.5)
            if not barFrame.Parent then break end
            TweenService:Create(glowStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.92}):Play()
            task.wait(1.5)
        end
    end
...
local UNLOCK_UI_SAVE_FILE = "VexisButtonsClean2_Pos.json"
...
function toggleUnlockBaseMenu(state)
    ...
    local UIC = {
        ...
        SURF = Color3.fromRGB(15, 15, 22),
        TEXT = Color3.fromRGB(255, 255, 255),
        AQUA_STROKE = Theme.Accent1,
        ACTIVE1 = Theme.GoldLight,
        ACTIVE2 = Theme.GoldDark,
        LOCK_RED_1 = Color3.fromRGB(170, 70, 80),
        LOCK_RED_2 = Color3.fromRGB(140, 55, 65),
        LOCK_GREEN_1 = Theme.GoldDark,
        LOCK_GREEN_2 = Theme.GoldLight,
    }
...
-- [Theme already defined at top - BLACK & GOLD]
...
local function ShowNotification(title, text)
    local existing = PlayerGui:FindFirstChild("VexisNotif")
    if existing then existing:Destroy() end

    local sg = Instance.new("ScreenGui", PlayerGui)
    sg.Name = "VexisNotif"; sg.ResetOnSpawn = false
...
-- FLOAT
-- ============================================================
local FLOAT_PLATFORM_NAME = "VEXIS_FLOAT_PLATFORM"
...
do
    ...
    local function createPlotBeam()
        ...
        plotBeam.Color = ColorSequence.new(Theme.Accent1)
...
local AutoStealColors = {
    BG = Theme.Background,
    SURF = Theme.Surface,
    SURF2 = Color3.fromRGB(25, 22, 28),
    TEXT = Theme.TextPrimary,
    DIM = Theme.TextSecondary,
    AQUA = Theme.Accent1,
    AQUA2 = Theme.Accent2,
    AQUA_STROKE = Theme.GoldLight,
    }
    ...
    local STEALBAR = {
        PANEL = Theme.Surface,
        PANEL2 = Theme.SurfaceHighlight,
        TEXT = Theme.TextPrimary,
        STROKE = Theme.GoldLight,
        GLOW = Theme.GoldLight,
        TRACK = Color3.fromRGB(26, 30, 44),
        TRACK2 = Color3.fromRGB(36, 42, 60),
        FILL1 = Theme.GoldLight,
        FILL2 = Theme.GoldDark,
    }
...
local targetOffsetY = Config.Positions.TargetControls and Config.Positions.TargetControls.OffsetY
    ...
    local MiniTargetColors = {
        ...
        AQUA2 = Theme.Accent2,
        AQUA_STROKE = Theme.GoldLight,
        GREEN1 = Theme.GoldDark,
        GREEN2 = Theme.GoldLight,
        GREEN_STROKE = Theme.GoldLight,
...
for i, pb in ipairs(petButtons) do
    ...
    if pb.rankBoxStroke then
        pb.rankBoxStroke.Color = isSelected and Theme.Accent2 or AutoStealColors.AQUA_STROKE
...
local function scanSinglePlot(plot)
    ...
    local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
        ...
        local duelStatusColor = isInDuel and Color3.fromRGB(255, 0, 0) or Theme.Success
        ...
        local color = mutationColors[normalizedMutation] or Theme.Accent1
        
        local existing = PlayerGui:FindFirstChild("VexisPriorityAlert")
        if existing then existing:Destroy() end
        
        local alertGui = Instance.new("ScreenGui")
        alertGui.Name = "VexisPriorityAlert"
    ...
    local function updateTracer()
        ...
        if hrp and targetPart then
            ...
            local MUT_COLORS_TRACE = {
                Cursed=Color3.fromRGB(200,0,0), Gold=Theme.Accent1,
...
local function BuildSettingsAndMobileUI()
    ...
    if IS_MOBILE then
        ...
        mobileGui.Name = "VexisMobileControls"
...
local VEXIS = {
    ...
    SURF2 = Color3.fromRGB(25, 22, 35),         -- Dark gold surface
    SIDE = Color3.fromRGB(20, 18, 28),          -- Sidebar black-gold
    SIDE2 = Color3.fromRGB(30, 26, 42),         -- Sidebar highlight
    TEXT = Color3.fromRGB(255, 255, 255),       -- Pure white text
    DIM = Color3.fromRGB(180, 180, 200),        -- Dimmed white-gold
    TOP = Theme.GoldLight,                     -- Light gold gradient
    MID = Theme.GoldDark,                       -- Mid gold gradient
    BOT = Theme.GoldDark,                       -- Dark gold gradient
    ACCENT = Theme.Accent1,                    -- Main gold accent
    ACCENT2 = Theme.Accent2,                   -- Secondary gold
    STROKE = Theme.GoldLight,                  -- Gold strokes
    GLOW = Theme.GoldLight,                    -- Gold glow
    }

    local function VexisCorner(parent, r)
        local u = Instance.new("UICorner")
        u.CornerRadius = UDim.new(0, r)
        u.Parent = parent        return u
    end

    local function VexisStroke(parent, thickness, color, transparency)
        local s = Instance.new("UIStroke")
        s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        s.Thickness = thickness or 1
        s.Color = color or VEXIS.STROKE
        s.Transparency = transparency or 0.5
        s.Parent = parent
        return s
    end

    local function VexisGradient(parent, rotation)
        local g = Instance.new("UIGradient")
        g.Rotation = rotation or 90
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, VEXIS.TOP),
            ColorSequenceKeypoint.new(0.50, VEXIS.MID),
            ColorSequenceKeypoint.new(1.00, VEXIS.BOT),
        })
        g.Parent = parent
        return g
    end

    local function VexisAccentGradient(parent)
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, VEXIS.ACCENT),
            ColorSequenceKeypoint.new(1.00, VEXIS.ACCENT2),
        })
        g.Parent = parent
        return g
    end

    local function VexisTween(obj, ti, props, style, dir)
    ...
    VEXIS.BG = Color3.fromRGB(8, 10, 18)
    VEXIS.SURF = Color3.fromRGB(18, 20, 32)
    VEXIS.SURF2 = Color3.fromRGB(30, 34, 50)
    VEXIS.SIDE = Color3.fromRGB(32, 38, 58)
    VEXIS.SIDE2 = Color3.fromRGB(46, 54, 80)
    VEXIS.TEXT = Color3.fromRGB(235, 240, 255)
    VEXIS.DIM = Color3.fromRGB(160, 170, 200)
    VEXIS.TOP = Theme.GoldLight
    VEXIS.MID = Theme.GoldDark
    VEXIS.BOT = Theme.GoldDark
    VEXIS.ACCENT = Theme.Accent1
    VEXIS.ACCENT2 = Theme.Accent2
    VEXIS.STROKE = Theme.GoldLight
    VEXIS.GLOW = Theme.GoldLight
    Theme.Background = VEXIS.BG
    Theme.Surface = VEXIS.SURF
    Theme.SurfaceHighlight = VEXIS.SURF2
    Theme.Accent1 = VEXIS.ACCENT
    Theme.Accent2 = VEXIS.ACCENT2
    Theme.TextPrimary = VEXIS.TEXT
    Theme.TextSecondary = VEXIS.DIM
    Theme.Success = VEXIS.ACCENT2
    Theme.Error = VEXIS.ACCENT2

    local function VexisPad(parent, l, t, r, b)
    ...
    local function VexisBgTriGradient(parent)
        local g = Instance.new("UIGradient")
        g.Rotation = 90
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, VEXIS.TOP),
    ...
    local function VexisKnobGradient(parent)
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Theme.GoldLight),
            ColorSequenceKeypoint.new(1.00, Theme.GoldDark),
        })
        g.Parent = parent
        return g
    end

    local function VexisRgbGradient(parent)
        local g = Instance.new("UIGradient")
        g.Rotation = 0
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Theme.GoldLight),
            ColorSequenceKeypoint.new(0.50, Theme.GoldDark),
            ColorSequenceKeypoint.new(1.00, Theme.GoldLight),
        })
        g.Parent = parent
        return g
    end
    ...
    sFrame.BackgroundColor3 = VEXIS.BG
    sFrame.BackgroundTransparency = 0.03
    sFrame.BorderSizePixel = 0
    sFrame.Parent = settingsGui
    VexisCorner(sFrame, 18)

    local outline = VexisStroke(sFrame, 2, Theme.Accent1, 0)
    outline.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    local glow = Instance.new("UIStroke")
    glow.Color = Theme.GoldLight
    glow.Thickness = 4
    ...
    sidebar.BackgroundColor3 = VEXIS.SURF
    sidebar.BackgroundTransparency = 0
    sidebar.BorderSizePixel = 0
    sidebar.Parent = sFrame
    VexisCorner(sidebar, 18)
    VexisStroke(sidebar, 1, VEXIS.STROKE, 0.50)

    local brand = Instance.new("Frame")
    brand.Name = "Brand"
    brand.Size = UDim2.new(1, -16, 0, 60)
    brand.Position = UDim2.fromOffset(8, 8)
    brand.BackgroundColor3 = VEXIS.SURF2
    brand.BackgroundTransparency = 0
    brand.BorderSizePixel = 0
    brand.Parent = sidebar
    VexisCorner(brand, 14)
    VexisStroke(brand, 1, VEXIS.STROKE, 0.45)
    ...
    orb.BackgroundColor3 = VEXIS.ACCENT2
    orb.BorderSizePixel = 0
    orb.Parent = brand
    VexisCorner(orb, 12)
    VexisAccentGradient(orb)
    VexisStroke(orb, 1, VEXIS.GLOW, 0.45)

    task.spawn(function()
        while orb.Parent do
            VexisTween(orb, 1.6, {Rotation = 8}, Enum.EasingStyle.Sine)
            task.wait(1.6)
    ...
    footer.BackgroundColor3 = VEXIS.SURF2
    footer.BackgroundTransparency = 0
    footer.BorderSizePixel = 0
    footer.Parent = sidebar
    VexisCorner(footer, 12)
    VexisStroke(footer, 1, VEXIS.STROKE, 0.48)
    ...
    footerText.Text = "VEXIS HUB v1.0"
    footerText.TextSize = 11
    footerText.TextColor3 = VEXIS.DIM
    ...
    sHeader.BackgroundColor3 = VEXIS.SURF
    sHeader.BackgroundTransparency = 0
    sHeader.BorderSizePixel = 0
    sHeader.Parent = right
    VexisCorner(sHeader, 16)
    VexisStroke(sHeader, 1, VEXIS.STROKE, 0.42)
    ...
    sTitle.Text = "VEXIS HUB"
    sTitle.TextColor3 = VEXIS.TEXT
    sTitle.TextSize = 21
    sTitle.TextXAlignment = Enum.TextXAlignment.Left
    sTitle.Parent = sHeader

    do
        local titleGradient = VexisRgbGradient(sTitle)
    ...
    sClose.BackgroundColor3 = VEXIS.SURF2
    sClose.BackgroundTransparency = 0
    sClose.BorderSizePixel = 0
    sClose.Text = "X"
    sClose.Font = Enum.Font.GothamBold
    sClose.TextSize = 13
    sClose.TextColor3 = VEXIS.TEXT
    sClose.Parent = sHeader
    VexisCorner(sClose, 10)
    VexisStroke(sClose, 1, VEXIS.STROKE, 0.36)

    sClose.MouseEnter:Connect(function()
        VexisTween(sClose, 0.16, {BackgroundTransparency = 0.00, BackgroundColor3 = Color3.fromRGB(64, 74, 108)})
    end)
    sClose.MouseLeave:Connect(function()
        VexisTween(sClose, 0.16, {BackgroundTransparency = 0.00, BackgroundColor3 = VEXIS.SURF2})
    end)
    ...
    local function makePage(name)
        ...
        card.Name = "Card"
        card.BackgroundColor3 = VEXIS.SURF
        card.BackgroundTransparency = 0
        card.BorderSizePixel = 0
        card.Size = UDim2.fromScale(1, 1)
        card.Parent = page
        VexisCorner(card, 16)
        VexisStroke(card, 1, VEXIS.STROKE, 0.48)
        ...
        sc.ScrollBarImageColor3 = VEXIS.ACCENT2
        ...
        do
            ...
            accent.BackgroundColor3 = VEXIS.ACCENT
            accent.BorderSizePixel = 0
            accent.Parent = row
            VexisCorner(accent, 3)
            VexisAccentGradient(accent)
            ...
            line.BackgroundColor3 = VEXIS.ACCENT2
            line.BackgroundTransparency = 0.35
            line.BorderSizePixel = 0
            line.Parent = row
            VexisCorner(line, 999)
        end
    ...
    local function makeSideTab(def, order)
        ...
        bg.BackgroundColor3 = VEXIS.SIDE2
        bg.BackgroundTransparency = 0.32
        bg.BorderSizePixel = 0
        bg.Parent = btn
        VexisCorner(bg, 11)

        local bgStroke = VexisStroke(bg, 1, VEXIS.STROKE, 0.52)

        local bar = Instance.new("Frame")
        bar.Size = UDim2.fromOffset(4, 10)
        bar.Position = UDim2.new(0, 7, 0.5, 0)
        bar.AnchorPoint = Vector2.new(0, 0.5)
        bar.BackgroundColor3 = VEXIS.ACCENT
        bar.BorderSizePixel = 0
        bar.Parent = bg
        VexisCorner(bar, 999)
        VexisAccentGradient(bar)
        ...
        bottomLine.BackgroundColor3 = VEXIS.ACCENT
        bottomLine.BorderSizePixel = 0
        bottomLine.Parent = bg
        VexisAccentGradient(bottomLine)

        btn.MouseEnter:Connect(function()
            if currentKey ~= def.key then
                VexisTween(bg, 0.16, {BackgroundTransparency = 0.09, BackgroundColor3 = Color3.fromRGB(52, 61, 90)})
                VexisTween(label, 0.16, {TextColor3 = VEXIS.TEXT})
                VexisTween(bottomLine, 0.16, {Size = UDim2.new(0.50, 0, 0, 1)})
            end
        end)

        btn.MouseLeave:Connect(function()
            if currentKey ~= def.key then
                VexisTween(bg, 0.16, {BackgroundTransparency = 0.32, BackgroundColor3 = VEXIS.SIDE2})
                VexisTween(label, 0.16, {TextColor3 = VEXIS.DIM})
                VexisTween(bottomLine, 0.16, {Size = UDim2.new(0.35, 0, 0, 1)})
            end
        end)
    ...
    showTab = function(key)
        ...
        for name, refs in pairs(tabButtons) do
            ...
            VexisTween(refs.ButtonBack, 0.20, {
                BackgroundTransparency = selected and 0.03 or 0.18,
                BackgroundColor3 = selected and Color3.fromRGB(56, 66, 98) or VEXIS.SIDE2
            })
            VexisTween(refs.Stroke, 0.20, {
                Transparency = selected and 0.18 or 0.52
            })
            VexisTween(refs.Bar, 0.20, {
                Size = selected and UDim2.fromOffset(4, 24) or UDim2.fromOffset(4, 10)
            })
            VexisTween(refs.Label, 0.20, {
                TextColor3 = selected and VEXIS.TEXT or VEXIS.DIM,
                Position = selected and UDim2.fromOffset(24, 0) or UDim2.fromOffset(22, 0)
            })
            VexisTween(refs.BottomLine, 0.20, {
                Size = selected and UDim2.new(1, -26, 0, 1) or UDim2.new(0.35, 0, 0, 1)
            })
        end

        local pages = _G.VexisPageScrolls
        if pages and pages[key] then
            _G.VexisActiveScroll = pages[key]
        else
            _G.VexisActiveScroll = sList
        end
    ...
    function _G.SetVexisUIPage(name)
        local pages = _G.VexisPageScrolls
        if pages and pages[name] then
            _G.VexisActiveScroll = pages[name]
            showTab(name)
        else
            _G.VexisActiveScroll = sList
        end
    ...
    local function CreateToggleSwitch(parent, initialState, callback)
        ...
        local function startPulse()
            ...
            task.spawn(function()
                while sw.Parent and isOn and pulseId == myId do
                    VexisTween(stGlow, 0.85, {Transparency = 0.08}, Enum.EasingStyle.Sine)
                    task.wait(0.9)
                    if not (sw.Parent and isOn and pulseId == myId) then break end
                    VexisTween(stGlow, 0.85, {Transparency = 0.20}, Enum.EasingStyle.Sine)
                    task.wait(0.9)
                end
            end)
        end
        ...
        btn.MouseButton1Click:Connect(function()
            callback(not isOn, function(state)
                ...
                VexisTween(stBase, 0.22, {Transparency = state and 0.85 or 0.60})
                VexisTween(knobStroke, 0.22, {Transparency = state and 0.35 or 0.65})
                VexisTween(sw, 0.22, {BackgroundTransparency = state and 0.00 or 0.02})
                if state then
                    isOn = true
                    startPulse()
                    VexisTween(stGlow, 0.22, {Transparency = 0.10})
                else
                    isOn = false
                    pulseId += 1
                    VexisTween(stGlow, 0.12, {Transparency = 1}, Enum.EasingStyle.Sine)
                end
            end)
        end)
    ...
    local function CreateRow(text, height)
        ...
        row.BackgroundColor3 = VEXIS.SURF2
        row.BackgroundTransparency = 0.12
        row.BorderSizePixel = 0
        row.Parent = (_G.VexisActiveScroll or sList)
        VexisCorner(row, s(10))
        VexisStroke(row, 1, VEXIS.STROKE, 0.55)
        VexisPad(row, s(12), s(8), s(12), s(8))
    ...
    local function CreatePageHeader(text)
        ...
        line.BackgroundColor3 = VEXIS.ACCENT
    ...
    do
    _G.SetVexisUIPage("Main")
    _G.VexisLastAutoTpLoadRow = CreateRow("Auto TP on Script Load")
    CreateToggleSwitch(_G.VexisLastAutoTpLoadRow, Config.TpSettings.TpOnLoad, function(ns, set)
    ...
    CreateToggleSwitch(rHideAdminPanel, Config.HideAdminPanel, function(ns, set)
        ...
        local adUI = PlayerGui:FindFirstChild("VexisAdminPanel")
    ...
    CreateToggleSwitch(rJoinerRow, Config.ShowJobJoiner, function(ns, set)
        ...
        local gui = PlayerGui:FindFirstChild("VexisJobJoiner")
    ...
    bReset.MouseButton1Click:Connect(function()
        ...
        if PlayerGui:FindFirstChild("VexisAdminPanel") and PlayerGui.VexisAdminPanel:FindFirstChild("Frame") then
            PlayerGui.VexisAdminPanel.Frame.Position = UDim2.new(
        ...
        if PlayerGui:FindFirstChild("VexisInvisPanel") and PlayerGui.VexisInvisPanel:FindFirstChild("Frame") then
            PlayerGui.VexisInvisPanel.Frame.Position = UDim2.new(
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if NormalizeKeyName(Config.JobJoinerKey, "J") and input.KeyCode == Enum.KeyCode[NormalizeKeyName(Config.JobJoinerKey, "J")] then
            local joinerGui = PlayerGui:FindFirstChild("VexisJobJoiner")
            if joinerGui then
...
task.spawn(function()
    ...
    local function cleanupLegacyBrainrotESP()
        for _, inst in ipairs(Workspace:GetChildren()) do
            if inst.Name == "VexisBrainrotESP" then
                pcall(function() inst:Destroy() end)
            end
        end
        for _, inst in ipairs(game:GetService("CoreGui"):GetChildren()) do
            if inst:IsA("Highlight") and inst.Name == "VexisBrainrotESP_Highlight" then
                pcall(function() inst:Destroy() end)
            end
        end
    ...
    brainrotESPFolder.Name = "VexisBrainrotESP"
    ...
    local function getMutationColor(mut)
        return (mut and mut ~= "None" and mut ~= "N/A" and MUT_COLORS[mut]) or Theme.Accent1
    end
    ...
    local function createBrainrotBillboard(data)
        ...
        local strokeColor = Theme.Accent2
    ...
    local function updateBrainrotBillboard(bb, data, hrp)
        ...
        if data and data.kind == "CONVEYOR" then
            if stroke then
                stroke.Color = hasMutation and color or Theme.Accent2
            end

            if tagLabel then
                tagLabel.Visible = true
                tagLabel.Text = "[CARPET]"
                tagLabel.TextColor3 = Theme.Accent1
            end
        ...
        if stroke then stroke.Color = hasMutation and color or Theme.Accent2 end
...
task.spawn(function()
    ...
    local INVIS_UI = {
        ...
        SLIDER_FILL1 = Theme.GoldLight,
        SLIDER_FILL2 = Theme.GoldDark,
    }
...
task.spawn(function()
    ...
    if PlayerGui:FindFirstChild("VexisStatusHUD") then PlayerGui.VexisStatusHUD:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "VexisStatusHUD"
    ...
    local function refreshVexisStatusHUD()
        local viewport = Camera and Camera.ViewportSize or Vector2.new(1920, 1080)
        local minScale = math.min(viewport.X / 1920, viewport.Y / 1080)
        hudScale.Scale = math.clamp(minScale, 0.72, 1)
    end

    refreshVexisStatusHUD()
    if Camera then
        Camera:GetPropertyChangedSignal("ViewportSize"):Connect(refreshVexisStatusHUD)
    end
    new("UICorner", {
    ...
    -- Vexis HUB HUD dragging disabled
...
task.spawn(function()
    ...
    local COLORS = {
        Text        = Theme.TextPrimary,
        SilverA     = Theme.GoldLight,
        SilverB     = Theme.GoldDark,
        SteelA      = Theme.Accent2,
        Stroke      = Theme.GoldLight,
        UnlockA     = Theme.GoldLight,
        UnlockB     = Theme.GoldDark,
    }
...
task.spawn(function()
    ...
    local function createMineESP(mine)
        ...
        textLabel.TextColor3 = Theme.Accent1
...
task.spawn(function()
    ...
    local C = {
        ...
        SELECT_FILL = Theme.GoldDark,
        CHIP = Theme.GoldDark,
        CHIP_HOVER = Theme.GoldLight,
        TEXT = Color3.fromRGB(243, 246, 255),
        DIM = Color3.fromRGB(167, 179, 218),
        DIM2 = Color3.fromRGB(127, 142, 187),
        LINE = Theme.GoldLight,
        LINE_SOFT = Theme.GoldDark,
    ...
    refreshRight = function()
        ...
        for i, itemName in ipairs(selectedOrder) do
            ...
            down.TextSize = 10
            down.TextColor3 = Theme.GoldLight
...
task.spawn(function()
    ...
    local function CreateInput(placeholder, width, default)
        ...
        s.Color = Theme.TextSecondary
        s.Thickness = 0.1
        s.Transparency = 0.6
        
        box.Focused:Connect(function() 
            TweenService:Create(s, TweenInfo.new(0.2), {Color = Theme.Accent1, Transparency = 0}):Play() 
        end)
        box.FocusLost:Connect(function() 
            TweenService:Create(s, TweenInfo.new(0.2), {Color = Theme.TextSecondary, Transparency = 0.6}):Play() 
        end)
    ...
    local joinBtn = CreateButton("JOIN", 60, Theme.Success)
    ...
    joinBtn.MouseButton1Click:Connect(function()
        ...
        isJoining = true
        joinBtn.Text = "STOP"
        joinBtn.BackgroundColor3 = Theme.Error
...
do
    ...
    apHighlight.Name = "VexisClickAPHighlight"
...
local function __cleanupResidualAdminPanels()
    ...
    local function shouldDestroyGui(gui)
        ...
        if gui.Name == "VexisAdminPanel" then return true end
...
local function __initAdminPanelLiteralReplacement()
    ...
    local RP_COLORS = {
        ...
        AccentSoft = Theme.GoldDark,
        AccentBright = Theme.GoldLight,
    ...
    rpStroke(rpui.backdrop, Theme.Accent1, 1, 0.82)
    ...
    rpStroke(rpui.topCard, Theme.Accent1, 1.2, 1)
    ...
    rpui.proxBtnStroke = rpStroke(rpui.proxBtn, Theme.Accent1, 1, 0.28)
    ...
    rpui.spamStroke = rpStroke(rpui.spamBaseBtn, Theme.Accent1, 1, 0.28)
    ...
    local function updateProxButton()
        ...
        rpui.proxBtnStroke.Color = enabled and Theme.GoldLight or Theme.Accent1
    end
    ...
    local function createUserTag(parent, kind, xOffset)
        ...
        local bgColor = kind == "kawaifu" and Color3.fromRGB(228, 91, 176) or Theme.Accent1
    ...
    local function createPlayerRow(plr)
        ...
        rpStroke(row, Theme.Accent1, 1, 0.98)
        ...
        rpStroke(avatarHolder, Theme.Accent1, 1, 0.72)
